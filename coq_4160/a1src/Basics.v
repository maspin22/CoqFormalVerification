...
(** There are now two symbols that look like equality: [=] and [=?].
    We'll have much more to say about the differences and similarities
    between them later.  For now, bear in mind that [=] is built-in to
    Coq itself, whereas [=?] is a notation we defined for [nat] and
    implemented ourselves as the function [eqb].  The standard library
    also defines and implements it in a similar way.  As [=], equality
    is something we can attempt to _prove_; but as [=?], it is something
    that Coq can _compute_.*)
...
Proof.
  simpl.
  intros f. intros H2. destruct b0.
...
Proof.
  simpl.
  intros f. intros H2. destruct b0.
  -rewrite -> H2. rewrite -> H2. rewrite negb_involutive. reflexivity.
  -rewrite -> H2. rewrite -> H2. rewrite negb_involutive. reflexivity.
 Qed.
...
(** Each SF chapter comes with a test file containing scripts that
    ...
    test files to make sure you haven't missed anything.  And if
    you're using SF as part of a course, your instructors will likely
    be using these test files to autograde your solutions.

    The test file for this chapter is [BasicsTest.v]. Make sure you
    have saved [Basics.v] to disk.  Then run [make BasicsTest.vo] in a
    terminal.  If you accidentally deleted an exercise or changed its
    name, then [make BasicsTest.vo] will fail with an error that tells
    you the name of the missing exercise.  Otherwise, you will get a
    lot of useful output:

    - First is all the output produced by [Basics.v] itself.  At the
      end of that you will see [COQC BasicsTest.v].
    ...
    - Second, for each required exercise, there is a report that tells
      ...
      If the _type_ is not [ok], it means you proved the wrong
      thing: most likely, you accidentally modified the theorem
      statement while you were proving it.  The autograder won't give
      you any points for that.  So, make sure to correct the theorem.

      The _assumptions_ are any unproved theorems which your
      solution relies upon.  "Closed under the global context" is a
      fancy way of saying "none": you have solved the
      exercise. (Congratulations!)  On the other hand, a list of
      axioms means you haven't fully solved the exercise. (But, see
      below regarding "Allowed Axioms".) If the exercise name itself
      is in the list, that means you haven't solved it; probably you
      have [Admitted] it.

    - Third, you will see the maximum number of points in standard and
      advanced versions of the assignment.  That number is the sum of
    ...
    Exercises that are manually graded will also show up in the output.
    But since they have to be graded by a human, the test script won't
    be able to tell you much about them.
*)

(* Sun Jan 19 22:39:40 EST 2020 *)
