...
Proof.
  intros n m H. split.
  -induction m. 
   + rewrite <- H. apply plus_n_O. 
   + rewrite <- plus_n_Sm in H. discriminate.
  -induction n.
   + rewrite <- H. reflexivity. 
   + rewrite plus_comm in H. rewrite <- plus_n_Sm with (n := m) (m := n) in H. discriminate.
Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H. unfold not. intro H'. intro Hp. 
  apply H in Hp. apply H' in Hp. destruct Hp.
Qed.
(** [] *)
...
Proof.
  intros X P H H'. destruct H' as [x E]. unfold not in E. apply E in H. destruct H.
Qed.
(** [] *)
...
Proof.
   intros X P Q. split.
   -intros [x  E]. destruct E as [Hp | Hq].
...
Proof.
  ...
  - induction l as [|x' l' IHl'].
    ...
    + simpl. intros [H | H].
      ...
      * apply IHl' in H. destruct H as [x [L R]]. exists x. 
        split. apply L. right. apply R.
  - induction l as [|x' l' IHl']. 
    + simpl. intros H. destruct H as [x [L R]]. destruct R.
    + simpl. intros H. destruct H as [x [L R]]. destruct R as [XS | IN].
      * left. rewrite XS. apply L.
      * right. apply IHl'. exists x. split. apply L. apply IN.
Qed.
(** [] *)
...
Proof.
  ...
  -split.
   ...
   +simpl. intros [contra | R]. 
    *destruct contra. 
    *apply R.
  -split.
   +simpl. intros [Hl | Hr]. 
    *left. left. apply Hl.
    *apply IH in Hr. destruct Hr as [H | H].
...
Theorem All_In :
  forall T (P : T -> Prop) (l : list T),
    ...
    All P l.
Proof.
  intros T P l. induction l as [|a' l' IH]. 
   - split.
...
Proof.
     ...
     -simpl. reflexivity.
     -simpl. rewrite <- IH.
      assert (forall (X: Type) (l l1 : list X) (x: X),
      rev_append l (l1 ++ [x]) = rev_append l l1 ++ [x] ).
      {intros Y l l1 y. generalize dependent l1.
       induction l as [| a l' IHl]. 
         * simpl. intros. reflexivity.
         * simpl. intros. apply (IHl (a::l1)).
      }
      apply (H X t [] x). 
Qed.
(** [] *)
...
Proof.
  ...
  Search "evenb_S". intros.
  induction n as [|n' IHn].
  - assert (evenb 0 = true). { reflexivity. } rewrite H. exists 0. reflexivity.
  - rewrite evenb_S. inversion IHn.  destruct (evenb n') in H. 
    + exists x. rewrite H. rewrite evenb_double. simpl. reflexivity.
    + assert (forall k, evenb (S (double k)) = false).
      { intros k. induction k as [|k' IHk'].
        - reflexivity.
        - simpl. apply IHk'.
      }
      rewrite H. rewrite H0.  simpl. exists (S x). reflexivity.
Qed.   
(** [] *)
...
Proof.
  ...
  -intros H. split.
   +  destruct b2. 
      * rewrite <- H. simpl. destruct b1. simpl. reflexivity. simpl. reflexivity.
      * rewrite <- H. destruct b1. simpl in H. discriminate. simpl in H. discriminate. 
   + destruct b1. 
      * rewrite <- H. simpl. reflexivity. 
      * rewrite <- H. destruct b2. simpl in H. discriminate. simpl. reflexivity.
...
Proof.
  ...
  -intros [H1 | H2]. rewrite H1.  simpl. reflexivity.
   destruct b1. rewrite H2.  simpl. reflexivity. rewrite H2.  simpl. reflexivity.
Qed.
(** [] *)
...
Theorem eqb_neq : forall x y : nat,
  x =? y = false <-> x <> y.
Proof.
  intros x y. rewrite <- not_true_iff_false with (b:= x =? y). unfold not . split.
  intros H. rewrite eqb_eq in H. apply H. intros H. 
...
Fixpoint eqb_list {A : Type} (eqb : A -> A -> bool)
  ...
  := match l1, l2 with 
     ...
     | [], _ => false
     | _, [] => false 
     | x :: lx' , y :: ly' => (eqb x y) && (eqb_list eqb lx' ly')
end.
...
Proof.
 intros A eqb H l1. induction l1 as [|x lx' IHx].
 -  intros l2. induction l2 as  [|y ly' IHy]. simpl. 
   + split. 
     * intros obv. reflexivity. 
     * intros obv. reflexivity.
   + simpl. split. 
 ...
 - intros l2. induction l2 as  [|y ly' IHy]. simpl. 
   + split. 
     ...
     * intro contra. discriminate contra.
   + split. 
     *  simpl. intros H'. apply andb_true_iff in H'. destruct H' as [Hl Hr]. 
        apply IHx with (l2:= ly') in Hr. apply H in Hl. rewrite Hl. rewrite Hr. 
        reflexivity.
     *  simpl. intros H'. apply andb_true_iff. split. 
        injection H'. intros. apply H. apply H1.
        apply IHx. injection H'. intros. apply H0.
Qed.
(** [] *)
...
Proof.
  intros X test l. induction l as [|x l' IH].
  - split.
    + simpl. intros. apply I.
    + simpl. intros. reflexivity.  
  - split.
    + simpl. intros. apply andb_true_iff in H. destruct H as [Hl Hr]. split.
      * apply Hl.
      * apply IH. apply Hr.
    + simpl. intros. apply andb_true_iff. destruct H as [Hl Hr]. split.
      * apply Hl.
      * apply IH. apply Hr.
Qed.
...
(** **** Exercise: 3 stars, standard (excluded_middle_irrefutable) 
    ...
    to add [P \/ ~P] as an axiom. Hint: you will need to come up with
    a clever assertion as the next step in the proof. *)
...
Proof.
  ...
  Search "double_neg". 
  assert (Hp: forall (P Q:Prop), (P \/ Q -> False) -> (P -> False) ). 
  { intros. destruct H0. left. apply H1. } 
  assert (Hq: forall (P Q:Prop), (P \/ Q -> False) -> (Q -> False) ). 
  { intros. destruct H0. right. apply H1. } 
  assert (Hpq: forall (P Q:Prop), (P \/ Q -> False) -> (P -> False) /\ (Q -> False)).
  { intros.  split. apply Hp with (Q:= Q). apply H0. 
    apply Hq with (P:= P0). apply H0. }
  apply Hpq in H. destruct H as [Hn Hnn]. 
  apply Hnn in Hn. destruct Hn.
Qed.
(** [] *)
