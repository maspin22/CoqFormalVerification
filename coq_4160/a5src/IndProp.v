...
Theorem ev_double : forall n,
  ev (double n).
Proof.
  intros n. induction n. apply ev_0.  apply ev_SS. apply IHn.
Qed.
(** [] *)
...
Theorem evSS_ev_remember : forall n,
  ev (S (S n)) -> ev n.
Proof.
  intros n H. remember (S (S n)) as k eqn:Heqk. destruct H as [|n' E'].
  - (* E = ev_0 *)
...
Theorem SSSSev__even : forall n,
  ev (S (S (S (S n)))) -> ev n.
Proof.
  intros n H. inversion H. apply evSS_ev' in H1. apply H1. Qed.
(** [] *)
...
Proof.
  intros n m E. induction E as [|n' E' IH].
  -simpl. intro H. apply H.
  -simpl. intro H. apply ev_SS. apply IH. apply H.
Qed.
(** [] *)
...
(* [R 1 1 2] is provable by c4
   [R 2 2 6] is not provable 
   -if we dropped the c5 contructor the set of provable propositions 
   would not change becuase c2 and c3 already allow for the 
   interchanging of m and n.
   -if we dropped constructor c4 from the definition of R, the 
   set of provable propositions would change because this realtion is
   not defined by the remaining constructors.
*)
...
Lemma MStar' : forall T (ss : list (list T)) (re : reg_exp T),
  ...
  fold app ss [] =~ Star re.
Proof.
  intros T ss re H. induction ss as [| x t IHss]. 
  - simpl. apply MStar0.
  - simpl. apply MStarApp.
    + apply H. simpl. left. reflexivity.
    + apply IHss. intros s.  simpl in H. destruct H with (s:=x). 
      * left. reflexivity.
      * intro H'. apply H. right. apply H'.
      * intro H'. apply H. right. apply H'.  
      * intro H'. apply H. right. apply H'.
      * intro H'. apply H. right. apply H'.
      * intro H'. apply H. right. apply H'.
...
Proof.
  ...
  - intro H. induction re.
    + inversion H.
    + simpl. exists []. apply MEmpty.
    + exists [t]. apply MChar.
    + simpl in H. apply andb_true_iff in H. destruct H as [Hl Hr].
      apply IHre1 in Hl. destruct Hl.  apply IHre2 in Hr. destruct Hr.
...
Proof.
 intros P b H. split.
 - destruct H. intro H'. reflexivity. unfold not in H. intro H'. apply H in H'.
   destruct H'.
 - destruct H eqn:E. intro H'. apply H0. intros. discriminate.
Qed.
(** [] *)
...
Theorem eqbP_practice : forall n l,
  count n l = 0 -> ~(In n l).
Proof.
  intros n l H. unfold not. intro H'. induction l as [|m l' IHl'].
  -simpl in H'. destruct H'.
  -simpl in H'. simpl in H. destruct (eqbP n m) as [H0 | H0]. 
   discriminate. unfold not in H0. destruct H' as [H1 | H2].
    +rewrite H1 in H0. apply H0. reflexivity.
    +apply IHl'. simpl in H. apply H. apply H2.
Qed.
(** [] *)
