...
(** The rule for assignment is the most fundamental of the Hoare
    ...
    Maybe coming up with that precondition seemed like it took some
    clever thought.  But there is a mechanical way we could have done
    it: if we take the postcondition [X = 1] and in it replace [X]
    with [Y]---that is, replace the left-hand side of the assignment
    statement with the right-hand side---we get the precondition, [Y =
    1]. *)
...
Proof.
  ...
  apply hoare_consequence_pre
    with (P' := (X < 5) [X |-> X + 1]).
  - apply hoare_asgn.
  - unfold "->>", assn_sub, t_update. intros st H.
    simpl in *. omega.
Qed.

(** Some of the lower-level reasoning we did in those proofs could
    be replaced by automation.  The manual manipulation of assertions is
    a good target.  Let's replace the final bullet of each proof
    with a single tactic that factors out the tactics used above. *)

Ltac assn_auto :=
  unfold "->>", assn_sub, t_update;
  try auto;  (* as in example 1, above] *)
  try (intros; simpl in *; omega). (* as in example 2 *)

(** Now we can redo the two proofs. *)

Example hoare_asgn_example1_auto :
  {{True}} X ::= 1 {{X = 1}}.
Proof.
  apply hoare_consequence_pre
    with (P' := (X = 1) [X |-> 1]).
...
Example assn_sub_example2_auto :
  {{X < 4}}
  X ::= X + 1
  {{X < 5}}.
Proof.
...
(** **** Exercise: 2 stars, standard (hoare_asgn_examples_2) 
    ...
    Use [eapply hoare_consequence_pre] to prove these triples. *)
...
Proof.
  ...
  apply hoare_asgn. intros.
  unfold assn_sub. simpl.  unfold assert_implies.
  simpl. intros. unfold t_update.
  simpl. destruct  (st X). omega. omega.
Qed.
...
Proof.
  ...
  simpl. eapply hoare_consequence_pre. apply hoare_asgn.
  unfold assn_sub. simpl. unfold assert_implies. intros.
...
Theorem swap_exercise :
  ...
  {{Y <= X}}.
Proof.
  Search "hoare_seq". apply hoare_seq with (Q := (Z <= Y )%assertion).
  apply hoare_seq with (Q := (Z <= X)%assertion).
  - eapply hoare_consequence_pre. apply hoare_asgn.
  unfold assn_sub. simpl. unfold assert_implies. intros. 
  unfold t_update. assumption. 
  - eapply hoare_consequence_pre. apply hoare_asgn.
  unfold assn_sub. simpl. unfold assert_implies. intros. 
  unfold t_update. assumption. 
  - eapply hoare_consequence_pre. apply hoare_asgn.
...
Proof.
  unfold not. intros. 
  unfold hoare_triple.
  specialize H with (a := 3) (n := 0).  eapply hoare_consequence_pre in H.
  apply hoare_asgn in H.
Admitted.
(** [] *)
...
(** As we did earlier, it would be nice to eliminate all the low-level
    ...
    [assn_auto] tactic we wrote isn't quite up to the job: *)
...
Proof.
  ...
  - (* Then *)
    eapply hoare_consequence_pre.
    + apply hoare_asgn.
    + assn_auto. (* fails to solve the goal *)
Abort.

(** Looking at the proof of [if_example], we can see why.  That proof
    needs to unfold a definition ([bassn]) and use a theorem
    ([eqb_eq]) that we didn't need in earlier proofs.  So, let's add
    those into our tactic --- and clean it up a little in the process:
    *)
...
(** **** Exercise: 2 stars, standard (if_minus_plus) 
    ...
    We provide a new tactic [assn_auto''] which extends our earlier
    tactic [assn_auto'] to handle inequalities. You should be able
    to use it to make quick work of this proof. *)

Ltac assn_auto'' :=
  unfold "->>", assn_sub, t_update, bassn;
  intros; simpl in *;
  try rewrite -> eqb_eq in *;
  try rewrite -> leb_le in *; (* this is the new line *)
  auto; try omega.
...
(** Add new evaluation rules to relation [ceval], below, for
    [IF1]. Let the rules for [TEST] guide you.*)
...
Example if1true_test :
  empty_st =[ IF1 X = 0 THEN X ::= 1 FI ]=> (X !-> 1).
Proof. (* FILL IN HERE *) Admitted.

Example if1false_test :
  (X !-> 2) =[ IF1 X = 0 THEN X ::= 1 FI ]=> (X !-> 2).
Proof. (* FILL IN HERE *) Admitted.
...
(** Invent a Hoare logic proof rule for [IF1].  State and prove a
    ...
    many one-sided conditionals as possible.
