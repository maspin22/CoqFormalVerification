...
as required by the first disjunct of (3). And knowing that [~ (X <=
Y)] ensures that subtracting [Y] from [X] and then adding back [Y]
produces [X], as needed by the second disjunct of (6). Note that [n -
m + m = n] does _not_ hold for arbitrary natural numbers [n] and [m]
(for example, [3 - 5 + 5 = 5]). *)
...
(** **** Exercise: 2 stars, standard (if_minus_plus_reloaded) 
    ...
    use 1)  y-x = y-x by reflexivity
    use 2) Z = Y - X ->> Y = X + Z by simpl algebra
    use 3)  x+z = x+z by reflexivity
...
(** Let's introduce a much more sophisticated tactic that will
    ...
    subgoals, tries to use several theorems about booleans and
    (in)equalities, then uses [eauto] and [omega] to finish off as
...
(** In order to give a specification to this program we need to
    ...
    is indeed the case:
      - (1) ->> (2):  trivial, because [True] implies anything.
...
(* 
      ...
      END
        ...
        use 1) 0=0  reflexivity
        use 2) x + 0 = m obvious
        use 3) 1 - 1 = 0 obvious
        use 4) X + Y =m /\ X = 0 -> 0 + Y = m -> Y = m
...
(** **** Exercise: 3 stars, standard (factorial) 
  ...
  END
    ...
    Briefly justify each use of [->>].
    use 1) x=m -> x! = m! -> 1*x! = m!
    use 2) unfold one iteration of factorial
    use 3) 0! = 1
...
(**
  ...
  WHILE ~(X = 0) && ~(Y = 0) DO
    ...
    Z := Z + 1
      {{ Z + min X Y = min a b }}
  END
    {{ Z + min X Y = min a b /\ min Y X = 0}} ->>
    {{ Z = min a b }}

    use 1) obvious
    use 2) use lemma one and two
    use 3) obvious
*)
...
Proof.
  verify.
  (* [verify_assn] didn't quite get all the way there *)
...
(** Using your solutions to [factorial] and [slow_assignment_dec] as a
    ...
    implements the factorial function.  Hint: recall the use of [ap] in
    assertions to apply a function to an Imp variable.
    ...
    rather than proving anything about your program.  Hint: if those
    two subgoals become tedious to prove, give some though to how
    you could restate your assertions such that the mathematical
    operations are more amenable to manipulation in Coq.  For example,
