...
(** That notion of _validity_ is based on the underlying model of how
...
(** At each step we have either used one of the rules, or we
    ...
    But, [{{X = 0}} SKIP {{X = 1}}] is not derivable, because there is no way
    to apply the rules to construct a proof tree. *)
...
Proof.
  ...
  - eapply H_Consequence_pre; constructor.
  - eapply H_Consequence_pre; try eapply H_Seq.
    apply IHc1. apply IHc2. intros s H. apply H.
  - eapply H_Consequence_pre. try constructor.
    apply IHc1. apply IHc2. intros; try apply H.
  - eapply H_Consequence; try apply H_While;
    try apply IHc; unfold assert_implies; intros; apply I.
Qed.
(** [] *)
...
Proof.
 induction c; intro Q.
 - eapply H_Consequence_pre. eapply H_Skip with (P := Q). intros. inversion H.
 - eapply H_Consequence_pre. eapply H_Asgn. intros. inversion H.
 - eapply H_Consequence_pre. eapply H_Seq. apply IHc1. apply IHc2. 
   intros. inversion H.
 -  eapply H_If; eapply H_Consequence_pre. 
    apply IHc1. intros. inversion H. inversion H0.
    apply IHc2. intros. inversion H. inversion H0.
 - eapply H_Consequence_post. eapply H_While. eapply H_Consequence_pre.
   apply IHc. intros. inversion H. inversion H0.
...
Proof.
 ...
 - apply hoare_skip.
 - apply hoare_asgn.
 - apply hoare_seq with (Q := Q). apply IHX2. apply IHX1.
 - apply hoare_if. apply IHX1. apply IHX2.
 - apply hoare_while. apply IHX.
 - eapply hoare_consequence. apply IHX. apply a. apply a0. 
Qed.
(** [] *)
...
Proof.
  intros. unfold wp. 
  unfold valid. intros. destruct H0. 
  apply H0. apply E_WhileTrue with (st' := st'). 
...
Proof.
  ...
  - eapply H_Consequence. eapply H_Skip. unfold assert_implies. 
    intros. eapply HT. apply E_Skip. apply H. unfold assert_implies. 
    intros. apply H.
  - eapply H_Consequence. eapply H_Asgn. unfold assert_implies. 
    intros. eapply HT. apply E_Ass. reflexivity. apply H. 
    unfold assert_implies. intros. apply H.
  - apply wp_seq. apply IHc1. 
    intros. unfold wp. intros.
      eapply HT. econstructor. eassumption. assumption. assumption.
    apply IHc2. unfold wp. intros. apply H0. apply H.
  - apply H_If. apply IHc1. intros. destruct H0. eapply HT.
    constructor. inversion H1. reflexivity. apply H. apply H0.
    apply IHc2.  intros. destruct H0. eapply HT. 
    eapply E_IfFalse. 3: apply H0. 2: apply H. 
    unfold bassn in H1. unfold not in H1. destruct (beval st b).
    exfalso. apply H1. reflexivity. reflexivity.
  -  eapply H_Consequence with (P' := wp (WHILE b DO c END) Q). 
     apply H_While. apply IHc.  
     eapply wp_invariant.  
     apply wp_is_weakest. assumption.
     unfold wp. unfold assert_implies. intros. destruct H. apply H.
     apply E_WhileFalse. unfold not in H0. unfold bassn in H0. 
     destruct (beval st b).  exfalso. apply H0. reflexivity. reflexivity.
Qed.  
(** [] *)
