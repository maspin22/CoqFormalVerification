...
(** **** Exercise: 2 stars, standard, recommended (add_inc_count) 

    Adding a value to a bag should increase the value's count by one.
    State that as a theorem and prove it. *)

Theorem bag_theorem : forall v : nat, forall s : bag, count v (add v s) = S (count v s). 
Proof.
...
Theorem rev_inv_lem : forall l1 l2 : natlist,
  rev (l1 ++ l2) = (rev l2) ++ (rev l1).
Proof.
  (* WORKED IN CLASS *)
  intros l1 l2. induction l1 as [| n l1' IHl1'].
  - simpl. (* l1 = nil *) rewrite -> app_nil_r.
    reflexivity.
  - (* l1 = cons *)
...
Proof.
  intros l. induction l as [|n l' IHl'].
  - reflexivity.
  - simpl.  rewrite -> rev_inv_lem. simpl. rewrite -> IHl'. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. (* l1 = nil *) rewrite -> app_assoc. 
    reflexivity.
  - (* l1 = cons *)
    simpl. rewrite -> IHl1'.   
    reflexivity. Qed.
...
Proof.
  simpl. destruct x. simpl. apply eqb_refl.
Qed.
(** [] *)
...
Proof.
 intros d x y o. simpl. intro H. rewrite -> H. reflexivity.
Qed.
(** [] *)
End PartialMap.
