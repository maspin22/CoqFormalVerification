...
(** If you don't recall selection sort or haven't seen it in
    awhile, see Wikipedia or read any standard textbook; some
...
(** A note on efficiency: Selection sort, like insertion sort,
    runs in quadratic time.  But selection sort typically makes many
    more comparisons than insertion sort, so insertion sort is usually
    preferable for sorting small inputs.  Selection sort can beat
    insertion sort if the cost of swapping elements is vastly higher
    than the cost of comparing them, but that doesn't apply to
    functional lists.  Bubble sort, by the way, is worse than both.
    See this video for a definitive statement:
...
(** Depending on the path you have followed through _Software
    Foundations_ it might have been awhile since you have worked with
...
Proof.
  ...
  - intros. inversion H. apply perm_skip. apply perm_nil.
  - intros. inv H. destruct (a >=? x). destruct (select x l) eqn:E.
    inversion H1. subst. apply IHl with (x := x :: [a]).
      change (x :: a ::l) with ([x] ++ [a] ++ l).
      change (n :: a ::l0) with ([n] ++ a :: l0).
      apply perm_trans with (l' := a :: [x] ++ l).
      apply perm_swap. apply Permutation_cons_app with (l := [x] ++ l).
      subst.
      change ([x] ++ l) with ([x] ++ [a] ++ l).
      change (n :: a ::l0) with ([n] ++ a :: l0).
      apply perm_trans with (l' := [a] ++ [x] ++l).
    destruct (select x (a::l)) eqn:E.  destruct (a >=? x). 
    + inversion H1.
(** [] *)
