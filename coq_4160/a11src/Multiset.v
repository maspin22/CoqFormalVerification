...
Proof.
  (* SOLUTION: *)
  intros.
  repeat rewrite union_assoc. rewrite (union_comm a b).
  reflexivity.
Qed.
...
Proof.
  (* SOLUTION: *)
  intros al bl H. induction H; simpl.
  - reflexivity.
  - rewrite IHPermutation. reflexivity.
  - apply union_swap.
  - transitivity (contents l'); assumption.
Qed.
...
Proof.
  ...
  specialize (H a). simpl in H. unfold union, singleton in H.
  bdestruct (a =? a).
  - inv H.
  - exfalso; auto.
Qed.
...
Proof.
  ...
  - unfold empty in H. inv H.
  - unfold union, singleton in H.
    bdestruct (x =? a).
    + subst. inv H.
      exists nil, l. split; auto.
    + destruct (IHl _ _ H) as [l1 [l2 [H1 H2]]].
...
(* SOLUTION: *)
  induction al; intros.
  - simpl in H; unfold empty in H. rewrite (contents_nil_inv _ H). constructor.
  - pose proof (H a). simpl in H0; unfold union, singleton in H0.
    bdestruct (a =? a); try (exfalso; auto; fail).
    destruct (contents_cons_inv _ _ _ H0) as [l1 [l2 [H3 H4]]]. subst.
    apply Permutation_cons_app.
    apply IHal. intros.
    specialize (H x). simpl in H; unfold union, singleton in H.
    bdestruct (x =? a).
      * subst.  rewrite H4; auto.
      * rewrite contents_insert_other in H; auto.
Qed.
(** [] *)
...
Proof.
  ...
  split; intros; specialize H with al; destruct H; split;
    try apply same_contents_iff_perm; assumption.
Qed.
